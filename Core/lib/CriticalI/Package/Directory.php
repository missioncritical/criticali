 ...
 */
class Vulture_Package_Directory {
  protected $directory;
  ...
  public function __construct($name, $dir) {
    ...
    $this->version = implode('.', Vulture_Package_Version::canonify_version($version));
    $this->name = $name;
  ...
  public function property($name, $default = null) {
    if (is_null($this->properties))
      $this->properties = Vulture_ConfigFile::read($this->directory."/package.ini");
  ...
  public function has_commands() {
    $glob = $this->property('command.glob', Vulture_Defaults::COMMAND_GLOB);
    return Vulture_Globber::match($this->directory, $glob) ? true : false;
  }
